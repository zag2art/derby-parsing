...
function parseViewElement(element) {
  ...
  var nameAttribute = element.attributes.is;
  var expression;
  if (!nameAttribute) {
  ...
  if (nameAttribute.expression) {
    expression = nameAttribute.expression;
  } else {
    var name = nameAttribute.data;
    expression = new expressions.LiteralExpression(name);
  }
  var viewAttributes = viewAttributesFromElement(element);
  var componentHooks = componentHooksFromAttributes(viewAttributes);
  var remaining = parseContentAttributes(element.content, viewAttributes);
  var viewInstance = createDynamicViewInstance(expression, viewAttributes, componentHooks.hooks, componentHooks.initHooks);
  finishParseViewElement(viewAttributes, remaining, viewInstance);
}

// TODO: Get rid of tag='foobar' functionality. Webpack doesn't support it.
// We will use only dynamic views, attributes and arrays (explicit)
...
function finishParseViewElement(viewAttributes, remaining, viewInstance) {
  if (!viewAttributes.hasOwnProperty('content') && remaining.length) {
    ...
    viewAttributes.content = (viewAttributes.within) ? template :
      new templates.ParentWrapper(template, undefined, true);
  } else if (viewAttributes.hasOwnProperty('content')){
    viewAttributes.content.isPartial = true;
  }
...
function parseContentAttributes(content, viewAttributes) {
  var remaining = [];
...
function parseAttributeElement(element, name, viewAttributes) {
  ...
  viewAttributes[camelName] = (element.attributes && element.attributes.within) ?
    content : new templates.ParentWrapper(content, undefined, true);
}
...
function parseViewExpression(expression) {
  ...
  // DynamicViewInstance gets its name at render time
  var viewInstance = createDynamicViewInstance(nameExpression, viewAttributes, componentHooks.hooks, componentHooks.initHooks);
  parseNode.content.push(viewInstance);
}
